...
class Merger {
    ...
    static public function downloadFile($ids, $isPlain = true){
        ...
        foreach($ids as $id){
            ...
            if($isPlain){
                $cmd = "cat {$word} {$silencePauseFile} {$word} {$silencePauseFile} {$translate} {$silencePauseFile} {$phrase} {$silencePauseFile} {$translatePhrase} {$silencePauseFile} >> {$resultFileName}";
            }
            else{
                $cmd = "cat {$translate} {$silencePauseFile} {$translatePhrase} {$silencePauseFile} {$word} {$silencePauseFile} {$phrase} {$silencePauseFile} {$silencePauseFile} >> {$resultFileName}";
            }
            shell_exec($cmd);
        }
